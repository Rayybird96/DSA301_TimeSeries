...
An MA(1) process is invertible to a AR(infinity) process, and an AR(infinity) process will have a PACF that is infinite in extent. (Recall that the PACF of an AR(k) process will show a sharp drop after k lags. Hence, when k is infinite, the PACF is infinite).
