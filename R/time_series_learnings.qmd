...
## 7.0 Preprosessing workflow

Workflow:
1. Remove deterministic variation (monthdays, inflation, etc.)

2. ts decomposition -> returns 2 things 
a. seasonality adjusted (trend + noise)
b. seasonal (run naive methods)

"usually" run box-cox on 2a
Model A: then run nsdiffs and ndiffs, then start to model BoxCox(2a) (Build Arima etc on this)
