...
sea_levels_df <- sea_levels_df |>
  dplyr::select(Year, `CSIRO Adjusted Sea Level`)|>
...
```{r}
# Create TS
green_model_ts <- ts(green_model_df[c("carbon_emissions", 
                                      "global_temperature", 
                                      "sea_level")],
                     start=c(1879,1), 
                     end=c(2013,1))
...
var3 <- VAR(green_model_ts_split$train[,1:3], p=3, type="const")
var_fc <- forecast(var3, h=outofsample)
accuracy(x=green_model_ts_split$test[,2:2], var_fc$forecast$global_temperature)
```

```{r}
var1 <- VAR(green_model_ts[,1:3], p=1, type="const")
serial.test(var1, lags.pt=10, type="PT.asymptotic")
```

```{r}
var2 <- VAR(green_model_ts[,1:3], p=2, type="const")
serial.test(var2, lags.pt=10, type="PT.asymptotic")
```

```{r}
var3 <- VAR(green_model_ts[,1:3], p=3, type="const")
